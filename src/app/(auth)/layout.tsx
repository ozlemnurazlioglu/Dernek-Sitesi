...
import type { AuthUiText, SiteSettings } from "@/lib/types";

export default function AuthLayout({ children }: { children: ReactNode }) {
  const { pageBlocks, siteSettings } = useStore();
  const auth =
    (pageBlocks["ui.auth"] as AuthUiText | undefined) ?? DEFAULT_AUTH_UI;
  const side = auth.sidePanel;
  const orgName = (siteSettings as SiteSettings).name;
      ...
      <aside className="hidden lg:block relative bg-brand-950 overflow-hidden">
        ...
        <div className="relative h-full flex flex-col justify-end p-12 text-white">
          <div className="max-w-md">
            ...
            <p className="mt-4 text-white/75">{side.description}</p>
          </div>
        </div>
      </aside>
    </div>
